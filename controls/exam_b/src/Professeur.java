...
public class Professeur {
    ...
    private int nombrCours;

    Professeur(String nom){
        this.nom = nom;
        this.listeCours = new String[10];
        moyenne = 0.0;
        nombrCours=0;
    }
    ...
    public void ajouterCours(String cours){
        if(listeCours.length<10) {
            // listeCours[listeCours.length+1]=cours;
            listeCours[nombrCours] = cours;
            nombrCours++;
            // calcule la moyenne
            calculeMoyenne();
        }
    ...
    public void calculeMoyenne(){
        if(nombrCours == 0) { // listeCours.length ==0
            moyenne = 0.0;
        }else{
           moyenne = nombrCours / 10;
        }
    }
    public String toString(){
        return nom + " (" + moyenne +")";
